...
const PillSlider: React.FC<IPillSlider> = ({ pills }) => {
	return <div className="flex pl-16 overflow-x-scroll overflow-y-visible pt-10">
		{pills.map((pill, index) => {
			return <div className="w-60 h-96 flex-shrink-0 mr-16 rounded-lg bg-slate-400 relative overflow-hidden transition-transform hover:-translate-y-2 hover:cursor-pointer" key={index}>
