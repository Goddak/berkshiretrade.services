...
const Hero: React.FC<IHero> = ({ title, subtitle, ctaLabel, ctaTo, imageSrc, imageAlt, fadeIn, fadeToClass, fadeBgClass, className }) => {
	...
	return <div className={`flex flex-col relative ${className}`} style={{height: "calc(100svh - 5rem)"}}>
		<div className="flex flex-col flex-grow justify-center items-center z-10">
			<h1 className="text-3xl font-light mb-4 md:text-4xl">{title}</h1>
			<h2 className="text-2xl font-light mb-4">{subtitle}</h2>
			<Link to={ctaTo} className="rounded-md bg-bts-black text-bts-yellow p-5 px-10 mt-5 md:text-2xl">{ctaLabel}</Link>
		</div>
